...
def draw_graph(data):
    ...
    plt.scatter(x, y)
    plt.ylabel('populatie (10k personen)')
    plt.xlabel('winst (10k$)')
    plt.show()
...
def compute_cost(X, y, theta):
    ...
    # YOUR CODE HERE
    # m is aantal datapunten, n is aantal features
    m, n = X.shape
    # voorspellingen = X * huidige waarden van theta
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for _ in range(num_iters):
        # bepaal voorspelling aan de hand van theta.T
        predictions = np.dot(X, theta_t)
        diff = predictions - y
        costs.append(compute_cost(X, y, theta_t))
        gradient = np.dot(X.T, diff) / m
        theta_t = theta_t - alpha * gradient
...
def draw_costs(data): 
    # OPGAVE 3b
    # YOUR CODE HERE
    # Eerste cost niet meenemen vanwege random waarden van theta
    plt.plot(data[1:])
...
def contour_plot(X, y):
    ...
    for i in range(len(t1)):
        for j in range(len(t2)):
            ...
            J_vals[i][j] = np.divide(compute_cost(X, y, theta), 100)
